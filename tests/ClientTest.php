...
class ClientTest extends TestCase
{
    ...
    public function testNormalizesDeprecatedAuthValuesBeforeHandler(): void
    {
        ...
        self::suppressDeprecations(static function () use ($client): void {
            $client->request('GET', 'http://example.com', ['auth' => [1, true, 123]]);
        });

        self::assertIsArray($capturedOptions);
        self::assertSame(['1', '1', '123'], $capturedOptions['auth']);
    }

    public function testNormalizesDeprecatedTlsFileOptionValuesBeforeHandler(): void
    {
        $capturedOptions = null;
        $client = new Client([
            'handler' => static function ($request, array $options) use (&$capturedOptions): FulfilledPromise {
        ...
        self::suppressDeprecations(static function () use ($client): void {
            $client->request('GET', 'http://example.com', [
                'cert' => [__FILE__, new ClientTestStringable('secret')],
                'ssl_key' => [__FILE__, new ClientTestStringable('secret')],
            ]);
        });

        self::assertIsArray($capturedOptions);
        self::assertSame([__FILE__, 'secret'], $capturedOptions['cert']);
        self::assertSame([__FILE__, 'secret'], $capturedOptions['ssl_key']);
    }

    public function testNormalizesDeprecatedStringOptionsBeforeHandler(): void
    {
        $capturedOptions = null;
        $client = new Client([
            'handler' => static function ($request, array $options) use (&$capturedOptions): FulfilledPromise {
        ...
        self::suppressDeprecations(static function () use ($client): void {
            $client->request('GET', 'http://example.com', [
                'cert_type' => new ClientTestStringable('pem'),
                'ssl_key_type' => new ClientTestStringable('pem'),
                'force_ip_resolve' => new ClientTestStringable('v4'),
            ]);
        });

        self::assertIsArray($capturedOptions);
        self::assertSame('pem', $capturedOptions['cert_type']);
        self::assertSame('pem', $capturedOptions['ssl_key_type']);
        self::assertSame('v4', $capturedOptions['force_ip_resolve']);
    }

    public function testNormalizesDeprecatedNumericStringOptionsBeforeHandler(): void
    {
        $capturedOptions = null;
        $client = new Client([
            'handler' => static function ($request, array $options) use (&$capturedOptions): FulfilledPromise {
        ...
        self::assertIsArray($capturedOptions);
        self::assertSame(1.5, $capturedOptions['timeout']);
        self::assertSame(2, $capturedOptions['connect_timeout']);
        self::assertSame(0.25, $capturedOptions['read_timeout']);
        self::assertSame(10, $capturedOptions['delay']);
    }

    public function testNormalizesDeprecatedIntegerOptionsBeforeHandler(): void
    {
        $capturedOptions = null;
        $client = new Client([
            'handler' => static function ($request, array $options) use (&$capturedOptions): FulfilledPromise {
        ...
        self::suppressDeprecations(static function () use ($client): void {
            $client->request('GET', 'http://example.com', ['retries' => '2']);
        });
