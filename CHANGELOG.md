...
- Wait for in-progress multiplex-capable (HTTP/2 or HTTP/3) connections by default (`multiplex` defaults to `prefer`) so concurrent cURL requests can share one connection
